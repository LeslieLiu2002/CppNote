...
> [!tip] `override` 关键字**只能出现在类定义内部的函数声明中**（头文件`.h`里），而**不能出现在类外部的函数定义/实现中**（源文件`.cpp`）。类似的关键字还有 `virtual`、`static` 以及参数默认值（比如 `int a = 0`）。注意`const`必须出现在类外实现中，因为`const`会改变函数的参数属性
## 虚表(vtable)
